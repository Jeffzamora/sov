...
try {
  ...
  // Horario del día (evita SQL dinámico / problemas de timezone)
  ...
  try {
    ensure_session();
    // En el sistema, la sesión usa: sesion_id_usuario
    $id_usuario = isset($_SESSION['sesion_id_usuario']) ? (int)$_SESSION['sesion_id_usuario'] : null;
  } catch (Throwable $e) {}

  $ins = $pdo->prepare("INSERT INTO tb_citas (id_cliente, fecha, hora_inicio, hora_fin, motivo, estado, id_usuario, fyh_creacion, fyh_actualizacion)
                        VALUES (:c,:f,:hi,:hf,:m,'programada',:u,NOW(),NOW())");
  $ok = $ins->execute([
    ':c'=>$id_cliente,
    ':f'=>$fecha,
  ...
  if (!$ok) throw new RuntimeException('No se pudo registrar la cita.');
