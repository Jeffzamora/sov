...
if (function_exists('require_perm')) require_perm($pdo, 'compras.crear', $URL . '/');
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2 align-items-center">
                <div class="col-sm-8">
                    <h1 class="m-0">
                        <i class="fas fa-cart-plus text-primary mr-2"></i>
                        Registro de una nueva compra
                        <span class="badge badge-light ml-2">Nuevo</span>
                    </h1>
                    <small class="text-muted">Selecciona un producto y un proveedor, completa los datos y guarda.</small>
                </div>
                <div class="col-sm-4 text-sm-right mt-2 mt-sm-0">
                    <a href="index.php" class="btn btn-outline-secondary">
                        <i class="fas fa-arrow-left"></i> Volver
                    </a>
                    <button type="button" class="btn btn-outline-danger" id="btnLimpiarForm">
                                                                       ...
                                                                       <td>
                                                                           ...
                                                                           <script>
                                                                               $('#btn_selecionar<?php echo $id_producto;?>').click(function () {
                                                                                   ...
                                                                                   // Precio editable en el panel de detalle
                                                                                   ...
                                                                                   // 1) quita el foco del botón actual
                                                                                   // 2) cierra el modal y luego enfoca un campo fuera del modal
                                                                                   ...
                                                                                   if (typeof window.recalcStockCompra === 'function') window.recalcStockCompra();
                                                                                   if (typeof window.recalcTotalesCompra === 'function') window.recalcTotalesCompra();
                                       ...
                                       <div class="col-md-12">
                                           <div class="form-group">
                                               ...
                                               <div class="input-group">
                                                   ...
                                                   <input type="number" step="0.01" min="0" class="form-control" id="precio_compra_controlador" placeholder="0.00">
                                               </div>
                                               <small class="text-muted">Puedes ajustar el precio de compra si corresponde.</small>
                                           </div>
                                       </div>
                                       ...
                                       <div class="col-md-12">
                                           ...
                                           <script>
                                               (function(){
                                                   ...
                                                   // input cubre teclado, pegar, flechas, scroll del mouse, etc.
                                   ...
                                   <script>
                                       $('#btn_guardar_compra').click(function () {
                                           ...
                                           SOV.ajaxJson({
                                               ...
                                               if (resp && resp.ok) {
                                                   SOV.toast('success', 'Compra registrada');
                                                   window.location.href = "../compras";
                                               } else {
                                                   SOV.toast('error', (resp && resp.error) ? resp.error : 'No se pudo registrar');
                                               }
                                           }).fail(function (xhr) {
...
<script>
    ...
    (function(){
        ...
        $(function(){
            // Fecha por defecto
            if (!$('#fecha_compra').val()) $('#fecha_compra').val(todayISO());

            // Botón limpiar
            $('#btnLimpiarForm').on('click', function(){
                $('#id_producto').val('');
                $('#codigo,#categoria,#nombre_producto,#usuario_producto').val('');
                $('#descripcio_producto').val('');
                $('#stock,#stock_minimo,#stock_maximo,#precio_compra,#precio_venta,#fecha_ingreso').val('');
                $('#stock_actual,#stock_total').val('');
                $('#precio_compra_controlador').val('');
                $('#cantidad_compra').val('');
                $('#total_compra').val('0.00');
                $('#comprobante').val('');

                $('#id_proveedor').val('');
                $('#nombre_proveedor,#celular,#telefono,#empresa,#email').val('');
                $('#direccion').val('');

                // Imagen default
                try { $('#img_producto').attr('src', <?php echo json_encode(product_image_url($imagen, $id_categoria ?? null, true)); ?>); } catch(e) {}
